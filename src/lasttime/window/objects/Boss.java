...
public class Boss extends GameObject {
 ...
 int fps=60;
   ...
   private Camera cam;
	...
	public Boss(float x, float y,Handler handler, ObjectId id,Camera cam) {
		super(x, y, id);
...
*/
	...
	public void render(Graphics g) {
		...
		g.drawImage(tex.lboss, (int)x, (int)y, null);
		...
		for(int i=0;i<handler.object.size();i++)
		{
			...
			if(tempObject.getX()>1000f)
			{
				...
				g.fillRect((int)((cam.getX())*(-1))	, 60, 90, 20);
				
				g.setColor(Color.red);
				g.fillRect((int)((cam.getX())*(-1))	, 60, Boss.this.hp*6, 20);
				
				g.setColor(Color.red);
				g.drawRect((int)((cam.getX())*(-1))	, 60, 90, 20);
				g.drawString("Boss0 HP",(int)(cam.getX())*(-1), 40);
			}
			}
			else if(tempObject.getId()==ObjectId.Player1&&tempObject.isAlive()==true)
			{
